...
class Learner(object):
  def __init__(self, n, l, L, N = 0, state = True):
    ...
    if state:
      self.model.add(Bidirectional(LSTM(l, return_sequences=True, stateful=state), batch_input_shape=(1,1,n)))
    else:
      self.model.add(Bidirectional(LSTM(l, return_sequences=True), input_shape=(1,n)))
